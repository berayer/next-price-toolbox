...
import { createColor, deleteColor } from '../action'
...
const columns = [
  ...
  columnHelper.accessor('name', { header: '花色' }),
  columnHelper.accessor('fullName', { header: '全称' }),
...
export function ColorTable({ colors }: { colors: Color[] }) {
  return (
    <GenericTable data={colors} columns={columns}>
      <CreateForm />
    </GenericTable>
  )
}
...
function DeleteButton({ id }: { id: number }) {
  const [_, invokeAction, isPending] = useActionState(deleteColor, null)
...
// 新增表单
function CreateForm() {
  const [state, formAction, isPending] = useActionState(createColor, null)
